...
function createPost($request_values)
	{
		...
		if (count($errors) == 0) {
			$user_id = $_SESSION['user']['id'];
			$query = "INSERT INTO posts (user_id, title, slug, views, image, body, published, created_at, updated_at) VALUES($user_id, '$title', '$post_slug','0', '$featured_image', '$body', $published, now(), now())";
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	function updatePost($request_values)
	{
		...
		// provide user with old image option incase that is not needed
